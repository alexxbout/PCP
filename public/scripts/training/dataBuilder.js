...
function build(sectionId, titre, contexte, missions, competences, productions) {
                    ...
                    <div class="flex items-center justify-between header relative">
                        ...
                        <div class="flex items-center justify-center font-cleanBold gap-x-2 m-5">
                            <div class="h-full w-max relative flex items-center justify-start mr-7">
                                <svg class="w-6 h-6 absolute text-global folder" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
    ...
    html += `</ul></div>`;
    if (productions != null)
        html += `<a class="p-2 text-center text-white duration-500 ease-in-out transform rounded-xl shadow-aroundColored bg-global font-cleanBold w-max hover:scale-95" target="_blank" href="${productions}">Productions</a></div></div>`;
    else
